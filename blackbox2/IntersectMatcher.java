...
public class IntersectMatcher extends InterUnionMatcher {
	
    public IntersectMatcher(HistoryPattern one, HistoryPattern two) {
        super(one, two);
    }
    ...
    public sat.Expr getSatExpr(BlackBox bb) {
        return new sat.And(getOne().getSatExpr(bb), getTwo().getSatExpr(bb));
    }
	...
	protected AbstractHistoryPattern reconstructed(ArrayList<AbstractHistoryPattern> children) {
    	...
    	return new IntersectMatcher((HistoryPattern)children.get(0), (HistoryPattern)children.get(1));
    }
}
